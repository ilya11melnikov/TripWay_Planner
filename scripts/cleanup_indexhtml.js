...
function main() {
  ...
  if (args.includes('--keep-public-only')) {
    // default is already public only
  } else if (args.includes('--keep-build')) {
    whitelist.add(path.normalize(path.join(repoRoot, 'build', 'index.html')));
  } else if (args.includes('--keep-src')) {
    whitelist.add(path.normalize(path.join(repoRoot, 'src', 'index.html')));
  }
  ...
  console.log('  --keep-src      keep src/index.html as well');
